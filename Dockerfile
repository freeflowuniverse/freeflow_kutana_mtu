...
COPY ./freeflowconnect_gateway /janus-gateway/
...
COPY ./freeflowconnect_gateway/plugins /opt/janus/lib/janus/plugins
...
COPY ./freeflowconnect_gateway/certs/* /opt/janus/share/janus/localcerts/
COPY ./freeflowconnect_gateway/certs/* /opt/janus/share/janus/certs/
COPY ./freeflowconnect_gateway/certs /certs
RUN cd libs/usrsctp && make install
RUN cd libs/libsrtp-2.1.0 && make install
RUN cd libs/libnice && make install

COPY ./freeflowconnect_gateway/configs/* /opt/janus/etc/janus/
COPY ./freeflowconnect_gateway/html/* /janus-gateway/html/
